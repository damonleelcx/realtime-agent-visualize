...
- G4.2 **`kline-viz` skill** renders the interactive self-contained HTML: ECharts candlestick (OHLC) + volume subplot, a marker for **every curated event** at its (snapped) trading day sized by impact — those that align to an inflection are emphasized — plus inflection markers colored **by `InflectionKind`**. Hover tooltip shows OHLCV + `fetched_at`; clicking an event marker opens a drill-down panel with the `CuratedEvent` rationale, impact rating, `lag_days` (when aligned), and **clickable `source_url` links** back to the raw `NewsItem` (可溯源). *(Post-build update: originally only inflection-aligned events were drawn; now all curated events appear.)*
